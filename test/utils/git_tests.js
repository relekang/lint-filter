...
+const matches = diff.match(/\@\@ -\d+,\d+ \+(\d+),(\d+) \@\@/g)
  `;
  t.deepEqual(gitUtils.parseDiffRanges(diff), [
    [8, 9],
    [47, 47],
  ]);
});

test('parseDiffRanges(diff) should return not match range if it is in the code diff', (t) =>
  t.deepEqual(gitUtils.parseDiffRanges('+@@ -8,27 +8,43 @@'), []));
...
test.serial(
  ...
  async (t) => {
    ...
    t.context.sandbox
      .stub(spawn, 'default')
      .returns(Promise.resolve(diffFixture));
...
test.serial(
  ...
  async (t) => {
    ...
    t.deepEqual(spawn.default.getCall(0).args, [
      'git',
...
test.serial(
  ...
  async (t) => {
    ...
    t.deepEqual(spawn.default.getCall(0).args, [
      'git',
      ['merge-base', 'origin/production', 'HEAD'],
    ]);
  }
);
